...
class _UserChatListState extends State<UserChatList> {
  ...
  Widget _buildChatTile(Map<String, dynamic> chatData, String chatId) {
    ...
    // Get the other participant (lawyer)
    final otherParticipantId = participants.firstWhere(
      (id) => id != currentUserId,
      orElse: () => '',
    );
    
    final otherParticipantName = participantNames[otherParticipantId] ?? 'Unknown Lawyer';
    final lastMessage = chatData['lastMessage'] ?? '';
    final lastMessageTime = chatData['lastMessageTime'] as Timestamp?;
    final unreadCount = (chatData['unreadCount'] as Map<String, dynamic>?)?[currentUserId] ?? 0;
    
    return FutureBuilder<DocumentSnapshot>(
      future: _firestore.collection('lawyers').doc(otherParticipantId).get(),
      builder: (context, lawyerSnapshot) {
        String lawyerName = otherParticipantName;
        ...
        if (lawyerSnapshot.hasData && lawyerSnapshot.data!.exists) {
          ...
          lawyerName = lawyerData['name'] ?? otherParticipantName;
              ...
              children: [
                CircleAvatar(
                  ...
                  child: lawyerProfileImage == null
                      ? const Icon(Icons.person, color: Color(0xFF353E55), size: 28)
                      : null,
                ),
                if (unreadCount > 0)
                  Positioned(
